...
class QuantileLoss(nn.Module):
    ...
    def forward(self, input, target):
        return self.loss(input[:,2], target, self.high) + self.loss(input[:,0], target, self.low) + self.loss(input[:,1], target, 0.5)
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "models/crimewithpred.pth")