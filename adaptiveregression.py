...
X_cal, X_val, y_cal, y_val = train_test_split(X_leftover, y_leftover, train_size=0.7, random_state=rs)
...
scores = np.abs(pred - y_cal_array)/U

N = len(scores)
print(N)
quantile = np.ceil((N + 1) * (1 - alpha))/N
q_hat = np.quantile(scores, quantile, method="higher")

with torch.no_grad():
    predictions = model(torch.Tensor(X_val)).numpy()

lower_val = predictions[:,0]
upper_val = predictions[:,2]
pred_val = predictions[:,1]
U_val = upper_val - lower_val
upper_conformal = pred_val + q_hat*U_val
lower_conformal = pred_val - q_hat*U_val
...
plt.tight_layout()
plt.savefig("graphs/NNregressionAdaptive_bad")
plt.show() 
